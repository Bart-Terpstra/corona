...
numb_ic_beds = 2500
icRatio = 0.05 # fraction of infected people needs IC
...
dr_w_original = dr_w
dr_w_new =  0.03            # Deat rate new

# parameters for non-working
...
dr_nw_original = dr_nw
dr_nw_new = 0.45            # Death new
...
OD_w = pd.read_excel(str(folder_name + '/sir_od_matrix/data_ang/od_matrix_working.xlsx') , index_col='origin')
OD_w = OD_w.to_numpy()
...
OD_nw = pd.read_excel(str(folder_name + '/sir_od_matrix/data_ang/od_matrix_no_working.xlsx'), index_col='origin')
...
for time_step in tqdm(range(days)):
    ...
    if(SIR_NW_sim[:,1].sum() * 0.05 > numb_ic_beds):
        print(' ')
        #print(str(SIR_NW_sim[:, 1]))
        #print(str(SIR_NW_sim[:, 1] * 0.05))
    ...
    if (infected_pop_norm_w[-1] + infected_pop_norm_nw[-1]) * N_k_w.sum() * 0.05 > 2500:
        dr_w = dr_w_new
        dr_nw = dr_nw_new
    else:
        dr_w = dr_w_original
        dr_nw = dr_nw_original
...
df_w.to_csv(str(folder_name + '/sir_od_matrix/data_ang/SIR_W_values_62_munies_SIR_W.csv'))

# Save the infections per municipality
df_map_I_w = pd.DataFrame(map_I_w)
np.save(str(folder_name + '/sir_od_matrix/data_ang/I_62_101_W'), map_I_w)
df_map_I_w.to_csv(str(folder_name + '/sir_od_matrix/data_ang/I_62_101_W.csv'))
...
print(str(folder_name + '/sir_od_matrix/data_ang/SIR_NW_values_62_munies_SIR_NW.csv'))

df_nw.to_csv(str(folder_name + '/sir_od_matrix/data_ang/SIR_NW_values_62_munies_SIR_NW.csv'))

# Save the infections per municipality
df_map_I_nw = pd.DataFrame(map_I_nw)
np.save(str(folder_name + '/sir_od_matrix/data_ang/I_62_101_NW'), map_I_nw)
...
df_all.to_csv(str(folder_name + '/sir_od_matrix/data_ang/SIR_ALL_values_62_munies_SIR_ALL.csv'))
...
np.save(str(folder_name + '/sir_od_matrix/data_ang/I_62_101_COMBINED'), map_I)
df_map_I = pd.DataFrame(map_I)
df_map_I.to_csv(str(folder_name + '/sir_od_matrix/data_ang/I_62_101_COMBINED.csv'))
