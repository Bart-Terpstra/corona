...
# Read the OD matrix of the 62 municipalities
OD = pd.read_csv('../data/df_od_matrix.csv', index_col='origin')
OD = OD.to_numpy()
thresh = 250000#1000       # determines where the outbreak starts
days = 100#50
floor_division = 100000#20
...
first_infections = np.where(SIR[:, 0]<=thresh, SIR[:, 0]//floor_division, 0)
...
dr_vec = np.full(locs_len, dr)
log.write('4.3) --- Death Rate vector (dr): '+str(dr_vec)+'\n\n')
...
map_I = np.array(SIR_nsim[:,1])
map_I = map_I.reshape(62,1)
...
for time_step in tqdm(range(days)):
    ...
    new_dead = dr * SIR_sim[:, 1]
    log.write('7.) --- New dead vector:  '+str(new_dead)+'\n\n')
    ...
    SIR_sim[:, 1] = SIR_sim[:, 1] + new_infect - new_recovered - new_dead  # Infected
    SIR_sim[:, 2] = SIR_sim[:, 2] + new_recovered               # Recovered
    SIR_sim[:, 3] = SIR_sim[:,3] + new_dead
    ...
    D = SIR_sim[:,3].sum()/N_k.sum()

    # The last two values must be equal
    print(round(S,4), round(I,4), round(R,4),  round(D,4), (S+I+R+D)*N_k.sum(), N_k.sum())
    log.write('temp.) --- Check number equality:  '+str([round(S,4), round(I,4), round(R,4),  round(D,4), (S+I+R+D)*N_k.sum(), N_k.sum()])+'\n\n')
    ...
    # map_I.append(SIR_sim[:,1]) # infected
    map_I = np.append(map_I, SIR_nsim[:,1].reshape(62,1), axis=1)
    # map_D.append(SIR_sim[:,3]) # dead
...
# Create a dataframe
data = {'Susceptible': susceptible_pop_norm, 'Infected': infected_pop_norm, 'Recovered': recovered_pop_norm, 'Dead': dead_norm}
df = pd.DataFrame(data)
df.to_csv('sir_values_62_munies_sird.csv')

# Save the infections per municipality
np.save('../data/I_62_101', map_I)

# Plotting
fig = plt.figure()
to_plot = np.stack([susceptible_pop_norm, infected_pop_norm, recovered_pop_norm, dead_norm]).T
plt.plot(t, to_plot)
plt.ylabel('Population fraction')
plt.xlabel('Time (days)')
plt.legend(['Susceptible', 'Infected', 'Recovered', 'Dead'])
plt.title(r'SIR Model with $a={}$, $b={}$, $y={}$, $dr={}$'.format(public_trans, beta, gamma, dr))
plt.show()
